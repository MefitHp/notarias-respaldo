 ...
 * http://tutorials.jenkov.com/java-howto
 * /replace-strings-in-streams-arrays-files.html
 * 
 * To use any kinf of marker, use this class.
 * 
 * @author Oleg Varaksin
 */
public class TokenMarkerReplacingReader extends Reader{
	...
	public TokenMarkerReplacingReader(final ITokenResolver resolver,
			final Reader source, final String tokenStartMarker,
		...
		if ((tokenStartMarker == null || tokenStartMarker.length() < 1)
			...
			throw new IllegalArgumentException(
					"Token start / end marker is null or empty");
		}
		...
		this.pushbackReader = new PushbackReader(source, Math.max(
				tokenStartMarker.length(), tokenEndMarker.length()));
	...
	public int read() throws IOException {
		...
		// read proper number of chars into a temp. char array in order to find
		// token start marker
		...
		if (!Arrays.equals(tmpTokenStartMarkerChars, tokenStartMarkerChars)) {
			if (countValidChars > 0) {
				pushbackReader.unread(tmpTokenStartMarkerChars, 0,
						countValidChars);
			}
		...
		// found start of token, read proper number of chars into a temp. char
		// array in order to find token end marker
		...
		System.out.println("BUFFER:::"+tokenBuffer);
		System.out.println("RESOLVED:::"+resolvedToken);
		return resolvedToken.charAt(resolvedTokenIndex++);
	}
	...
	public int read(CharBuffer target) throws IOException {
		throw new UnsupportedOperationException(
				"Method int read(CharBuffer target) is not supported");
	}

	@Override
	public long skip(long n) throws IOException {
		throw new UnsupportedOperationException(
				"Method long skip(long n) is not supported");
	}
	...
	public void reset() throws IOException {
		throw new UnsupportedOperationException(
				"Method void reset() is not supported");
	}
}
