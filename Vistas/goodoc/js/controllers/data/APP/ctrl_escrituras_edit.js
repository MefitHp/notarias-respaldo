function ctrl_escrituras_edit($scope, $route, conexion_app, catalogos, $rootScope, $timeout) {
    ...
    $scope.fechacertificadovalido=false;
            ...
            function(data){
               if(data.exito){
                   if(data.fechacertificado !=null && data.fechacertificado!=""){
                        ...
                        var fecha = data.fechacertificado.split("-");
                        var moment_certificado = moment(fecha[1]+"-"+fecha[0]+"-"+fecha[2]);
                        $scope.fechacertificado = moment_certificado.format('DD-MMMM-YYYY');
                        $scope.fechaterminocertificado = moment_certificado.add("days",90).format('DD-MMMM-YYYY');
                        $scope.restodias = moment_certificado.diff(new Date(),"days");
                        $scope.fechacertificadovalido = moment_certificado.add("days",90).isAfter(new Date());
